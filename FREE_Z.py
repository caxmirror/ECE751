...
#
class myNode():
    def __init__(self, nodeid, bs, datasize,maxHeight=5):
        self.nodeid = nodeid
...
#
class myDummyNode():
    def __init__(self, nodeid, bs, x, y, z , dist, datasize):
        self.nodeid = nodeid
        self.bs = bs
        self.buffer = datasize
        self.skewrate = 15e-6
...
for i in range(0,nrNodes):
    ...
    for chanl in range(1,nodes[i].s1parameters.chanls):
        ...
        node = myDummyNode(nodes[i].nodeid, nodes[i].bs, nodes[i].x, nodes[i].y, nodes[i].z, nodes[i].dist, nodes[i].buffer)
